...
dataPath = path.join(dataPath, "GreaterDiscord") + "/";

let hasCrashed = false;
export default class GreaterDiscord {
    ...
    static async injectRenderer(browserWindow) {
        ...
        const success = await browserWindow.webContents.executeJavaScript(`
            ...
            //# sourceURL=greaterdiscord/renderer.js
        `);
    ...
    static setup(browserWindow) {
        ...
        process.env.GREATERDISCORD_DATA_PATH = dataPath;
        ...
        browserWindow.webContents.on("dom-ready", () => {
            ...
            electron.dialog.showMessageBox({
                ...
                detail: "GreaterDiscord has automatically disabled itself just in case. To enable it again, restart Discord or click the button below.\n\nThis may have been caused by a plugin. Try moving all of your plugins outside the plugin folder and see if Discord still crashed.",
    ...
    static disableMediaKeys() {
        if (!GreaterDiscord.getSetting("general", "mediaKeys")) return;
...
if (GreaterDiscord.getSetting("developer", "reactDevTools")) {
