...
function AddonError({err, index}) {
    ...
    function renderErrorBody() {
        ...
        return <div className="gd-addon-error-body">
            <Divider />
            <div className="gd-addon-error-stack">
                {Parser ? Parser.codeBlock.react({content: stack, lang: "js"}, null, {}) : stack}
            </div>
        </div>;
    }

    return <div key={`${err.type}-${index}`} className={joinClassNames("gd-addon-error", (expanded) ? "expanded" : "collapsed")}>
        <div className="gd-addon-error-header" onClick={toggle} >
            <div className="gd-addon-error-icon">
                {err.type == "plugin" ? <Extension /> : <ThemeIcon />}
            </div>
            <div className="gd-addon-error-header-inner">
                <Text tag="h3" size={Text.Sizes.SIZE_16} color={Text.Colors.HEADER_PRIMARY} strong={true}>{err.name}</Text>
                <div className="gd-addon-error-details">
                    <svg className="gd-addon-error-details-icon" aria-hidden="false" width="16" height="16" viewBox="0 0 12 12">
            ...
            <svg className="gd-addon-error-expander" width="24" height="24" viewBox="0 0 24 24">
...
export default function AddonErrorModal({transitionState, onClose, pluginErrors, themeErrors}) {
    ...
    return <ModalRoot transitionState={transitionState} className="gd-error-modal" size={ModalRoot.Sizes.MEDIUM}>
            <Header className="gd-error-modal-header">
                <Flex direction={Flex.Direction.VERTICAL}>
                    ...
                    <div className="gd-tab-bar">
                        {tabs.map(tab => <div onClick={() => {switchToTab(tab.id);}} className={joinClassNames("gd-tab-item", tab.id === selectedTab.id && "selected")}>{tab.name}</div>)}
                    </div>
                </Flex>
            </Header>
            <Content className="gd-error-modal-content">
                <div className="gd-addon-errors">
                    {selectedTab.errors.map((error, index) => <AddonError index={index} err={error} />)}
                </div>
            </Content>
            <Footer className="gd-error-modal-footer">
